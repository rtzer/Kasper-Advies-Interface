...
export default function ChannelPhonePage() {
  ...
  const phoneInteractions = data?.results.filter(int => int.kanaal === 'Telefoon') || [];
  
  // Apply type filter
  const filteredInteractions = phoneInteractions.filter(int => {
    if (filterType === 'all') return true;
    return int.type === filterType;
  });
  
  // Calculate total call duration
  const totalDuration = phoneInteractions.reduce((sum, int) => sum + (int.duur || 0), 0);
    ...
    <div className="p-6 space-y-6 bg-ka-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <span className="text-3xl">{getChannelIcon('Telefoon')}</span>
          <div>
            <h1 className="text-2xl font-bold text-ka-navy dark:text-white">Telefoon Gesprekken</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl font-bold text-ka-green">
            {phoneInteractions.filter(int => int.type === 'Inbound').length}
          </div>
        </Card>
        <Card className="p-4">
          <div className="text-sm text-ka-gray-500 dark:text-gray-400 mb-1">Uitgaand</div>
          <div className="text-2xl font-bold text-ka-navy dark:text-white">
            {phoneInteractions.filter(int => int.type === 'Outbound').length}
          </div>
        </Card>
        <Card className="p-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <span className="text-6xl mb-4 block">{getChannelIcon('Telefoon')}</span>
          <p className="text-ka-gray-500 dark:text-gray-400">Geen telefoon gesprekken gevonden</p>
        </div>
      ) : (
