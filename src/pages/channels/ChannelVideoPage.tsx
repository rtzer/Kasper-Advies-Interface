...
export default function ChannelVideoPage() {
  ...
  // Filter for Video/Zoom only
  const videoInteractions = data?.results.filter(int => int.kanaal === 'Zoom') || [];
  
  // Apply type filter
  const filteredInteractions = videoInteractions.filter(int => {
    if (filterType === 'all') return true;
    return int.type === filterType;
  });
  
  // Calculate total meeting duration
  const totalDuration = videoInteractions.reduce((sum, int) => sum + (int.duur || 0), 0);
    ...
    <div className="p-6 space-y-6 bg-ka-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <span className="text-3xl">{getChannelIcon('Zoom')}</span>
          <div>
            <h1 className="text-2xl font-bold text-ka-navy dark:text-white">Video Gesprekken</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4">
          <div className="text-sm text-ka-gray-500 dark:text-gray-400 mb-1">Totaal meetings</div>
          <div className="text-2xl font-bold text-ka-navy dark:text-white">
            {videoInteractions.length}
          </div>
        </Card>
        <Card className="p-4">
          <div className="text-sm text-ka-gray-500 dark:text-gray-400 mb-1">Inkomend</div>
          <div className="text-2xl font-bold text-ka-green">
            {videoInteractions.filter(int => int.type === 'Inbound').length}
          </div>
        </Card>
        <Card className="p-4">
          <div className="text-sm text-ka-gray-500 dark:text-gray-400 mb-1">Uitgaand</div>
          <div className="text-2xl font-bold text-ka-navy dark:text-white">
            {videoInteractions.filter(int => int.type === 'Outbound').length}
          </div>
        </Card>
        <Card className="p-4">
          <div className="text-sm text-ka-gray-500 dark:text-gray-400 mb-1 flex items-center">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <span className="text-6xl mb-4 block">{getChannelIcon('Zoom')}</span>
          <p className="text-ka-gray-500 dark:text-gray-400">Geen video gesprekken gevonden</p>
        </div>
      ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <Badge className={
                        ...
                        {int.type === 'Inbound' ? 'Inkomend' : 'Uitgaand'}
                      </Badge>
                      {int.duur && (
