...
export default function ChannelEmailPage() {
  ...
  const emailInteractions = data?.results.filter(int => int.kanaal === 'E-mail') || [];
  
  // Apply type filter
  const filteredInteractions = emailInteractions.filter(int => {
    ...
    <div className="p-6 space-y-6 bg-ka-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <span className="text-3xl">{getChannelIcon('E-mail')}</span>
          <div>
            <h1 className="text-2xl font-bold text-ka-navy dark:text-white">E-mail Gesprekken</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="p-4">
          <div className="text-sm text-ka-gray-500 dark:text-gray-400 mb-1">Ontvangen</div>
          <div className="text-2xl font-bold text-ka-green">
            {emailInteractions.filter(int => int.type === 'Inbound').length}
          </div>
        </Card>
        <Card className="p-4">
          <div className="text-sm text-ka-gray-500 dark:text-gray-400 mb-1">Verzonden</div>
          <div className="text-2xl font-bold text-ka-navy dark:text-white">
            {emailInteractions.filter(int => int.type === 'Outbound').length}
          </div>
        </Card>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <span className="text-6xl mb-4 block">{getChannelIcon('E-mail')}</span>
          <p className="text-ka-gray-500 dark:text-gray-400">Geen e-mail gesprekken gevonden</p>
        </div>
      ) : (
